...
def parse_absolute_time(text):
    ...
    days_of_week = {
        ...
        'пятница': 4, 'в пятницу': 4,
        'суббота': 5, 'в субботу': 5,
...
def handler_natural_language(message):
    ...
    reminder = create_reminder(chat_id, reminder_text, reminder_time, user_data[chat_id])
    if reminder:
        response = f'''
    Напоминалка создана ( •̀ ω •́ )✧
...
'''
        ...
        bot.send_message(chat_id, 'Ниче не получилось. Проверь правильность времени')
